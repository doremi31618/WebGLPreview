...
class LoadInit {
	...
	loadPano(file) {
		...
		const pano = loader.load(imageURL);
		pano.mapping = THREE.EquirectangularReflectionMapping;
		pano.encoding = THREE.sRGBEncoding;
		envMaps ["reflection"] = pano;
		Scene.skybox.material.map = pano;
		updateAllEnvTexture('envMap', envMaps);
	}
}
